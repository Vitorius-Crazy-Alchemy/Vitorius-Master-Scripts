...
cls
echo.
echo %cBlue%   ____ ____      _    ______   __    _    _     ____ _   _ _____ __  __ __   __ %cReset%
echo %cBlue%  / ___^|  _ \    / \  ^|__  /\ \ / /   / \  ^| ^|   / ___^| ^| ^| ^| ____^|  \/  ^|\ \ / / %cReset%
echo %cBlue% ^| ^|   ^| ^|_) ^|  / _ \   / /  \ V /   / _ \ ^| ^|  ^| ^|   ^| ^|_^| ^|  _^| ^| ^|\/^| ^| \ V /  %cReset%
echo %cBlue% ^| ^|___^|  _ ^<  / ___ \ / /_   ^| ^|   / ___ \^| ^|__^| ^|___^|  _  ^| ^|___^| ^|  ^| ^|  ^| ^|   %cReset%
echo %cBlue%  \____^|_^| \_\/_/   \_\____^|  ^|_^|  /_/   \_\_____\____^|_^| ^|_^|_____^|_^|  ^|_^|  ^|_^|   %cReset%
echo.
echo %cBlue%   We Are One, We Are Many, We Are Crazy Alchemy 332%cReset%
echo.
echo %cRed%[INFO] Loading creator Vitorius configurations version 1.33%cReset%
...
echo %cBlue%28. RAM Purge (Deep API Working Set Clear)%cReset%
echo %cBlue%29. Exit%cReset%
echo ======================================================
...
if "%choice%"=="28" goto op28
if "%choice%"=="29" goto exit_routine
goto menu
...
:: --- OPTION 28:RAM PURGE (WORKING SETS & CACHE) ---
:op28
echo.
echo %cRed%[INFO] Initializing Godlike RAM Purge...%cReset%
echo %cRed%[BACKGROUND] Forcing Windows Memory Manager to immediately flush process Working Sets, run garbage collection, and release dormant memory back to the physical pool.%cReset%

echo %cRed%[WORKING] Generating C# P/Invoke payload for native API access...%cReset%
set "ps1=%TEMP%\rampurge.ps1"
echo using System; > "%ps1%"
echo using System.Runtime.InteropServices; >> "%ps1%"
echo public class MemPurge { >> "%ps1%"
echo     [DllImport("psapi.dll")] >> "%ps1%"
echo     public static extern int EmptyWorkingSet(IntPtr hwProc); >> "%ps1%"
echo } >> "%ps1%"
echo Add-Type -TypeDefinition (Get-Content "%ps1%" -Raw) >> "%ps1%"
echo Get-Process ^| ForEach-Object { try { [MemPurge]::EmptyWorkingSet($_.Handle) ^| Out-Null } catch {} } >> "%ps1%"
echo [GC]::Collect() >> "%ps1%"
echo [GC]::WaitForPendingFinalizers() >> "%ps1%"
...
echo %cRed%[SUCCESS] RAM Purge Complete. Task Manager will show a massive drop in used memory.%cReset%
pause
goto menu
